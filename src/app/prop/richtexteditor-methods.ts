...
export class RichTextEditorMethods {
    private methods: { [Key: string]: string | object }[];
    private rte: RichTextEditor;
    private element: HTMLElement;

    constructor(rte: RichTextEditor, method: { [Key: string]: string | object }[]) {
        this.rte = rte;
        this.methods = method;
    }
    ...
    private submitAction(e: Event) {
        ...
        if (child.length > 0) {
            returnData = this.rte[methodName](...this.getParams(child,methodName));
        } else {
            returnData = this.rte[methodName].apply(this.rte);
        }
