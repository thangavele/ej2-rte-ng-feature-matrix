...
export class RichTextEditorProperty {
    private rte: RichTextEditor;
    private properties: (simple | complex)[];
    private element: HTMLElement;
    constructor(rte: RichTextEditor, properties: (simple | complex)[]) {
        this.rte = rte;
            ...
            if (ul && ul.getAttribute('data-prop') !== null) {
                if (e.target.type === 'checkbox') {
                    ...
                    if (options.length) {
                        ...
                        this.rte[ul.getAttribute('data-prop')] = temp;
                    }
                    this.rte.dataBind();
                }
            }
    ...
    private booleanProperty(prop: string, def: boolean): CheckBoxOptions {
        return {
            ...
            default: def,
            change: (e: Event) => {
                this.rte[prop] = (e.srcElement as HTMLInputElement).checked;
                this.rte.dataBind();
            }
        }
    }
    ...
    private ddlProperty(prop: string, def: string, data: string[]): DDLOptions {
        return {
            ...
            default: def,
            change: (e: Event) => {
                this.rte[prop] = (e.srcElement as HTMLSelectElement).value;
                this.rte.dataBind();
            }
        }
    }
    ...
    private buttonProperty(prop: string, def: string | boolean): ButtonOptions {
        return {
            ...
            text: prop,
            click: (e: Event) => {
                this.rte[prop] = (e.srcElement as HTMLInputElement).value;
                this.rte.dataBind();
            }
        }
    }
    ...
    private generateRow(data: Util.simple | Util.complex): rowOptions {
        ...
        if (!this.isComplex(data)) {
            ...
            let comp = new composite(data, this.rte);
